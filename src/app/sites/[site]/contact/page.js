...
const page = async ({ params }) => {
    ...
    // 3. Extract Data
    const { email, facebook, instagram, tiktok, whatsapp, phone, twitter, linkedin, address } = store.website || {};
    const main_color = store.main_color || '#4F46E5';

    // 4. Helper function to check if a value exists
    const hasVal = (val) => val && val.length > 0;
        ...
        <div className="min-h-screen bg-gray-50 font-sans" dir="rtl">
            ...
            <div className="container mx-auto px-4 py-12 max-w-5xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    ...
                    {hasVal(address) && (
                        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center hover:shadow-md transition-shadow group lg:col-span-3">
                            <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4 bg-gray-50 group-hover:scale-110 transition-transform">
                                <MapPin size={32} style={{ color: main_color }} />
                            </div>
                ...
                {(hasVal(facebook) || hasVal(instagram) || hasVal(tiktok) || hasVal(twitter)) && (
                    <div className="text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-4">
                            ...
                            {hasVal(twitter) && (
                                <a href={twitter} target="_blank" rel="noopener noreferrer" className="p-4 bg-white rounded-full shadow-sm border border-gray-200 hover:text-blue-400 hover:border-blue-400 transition-all hover:-translate-y-1">
                                    <Twitter size={28} />
                                </a>
                            )}
                            {hasVal(linkedin) && (
                                <a href={linkedin} target="_blank" rel="noopener noreferrer" className="p-4 bg-white rounded-full shadow-sm border border-gray-200 hover:text-blue-700 hover:border-blue-700 transition-all hover:-translate-y-1">
                                    <Linkedin size={28} />
                                </a>
                            )}
                        </div>
